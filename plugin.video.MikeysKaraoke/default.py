...
K_db='http://mikey1234-repo.googlecode.com/svn/addons/plugin.video.MikeysKaraoke/Karaoke.db'
updatetxt='http://mikey1234-repo.googlecode.com/svn/addons/plugin.video.MikeysKaraoke/update.txt'
...
def SEARCH(url):
        ...
        for links in link:
            if not '__video_id__' or not '__title__' in links:
                match=re.compile('dir="ltr".+?title="(.+?)".+?data-sessionlink=".+?".+?href=".+?=(.+?)"',re.DOTALL).findall(links)
                for items in match:
                    url= match[0][1]
                    name=match[0][0]
...
def TITLE_ORDERS_YOUTUBE(name, url,fanart):
        ...
        for links in link:
            if not '__video_id__' or not '__title__' in links:
                match=re.compile('title="(.+?)".+?href="/watch.+?=(.+?)"',re.DOTALL).findall(links)
                for items in match:
                    url= match[0][1]
                    name=match[0][0]
                    name = str(name).replace("&#39;","'") .replace("&amp;","and") .replace("&#252;","u") .replace("&quot;","").replace("[","").replace("]","").replace("-"," ")
        ...
        for links in link:
            if not '__video_id__' or not '__title__' in links:
                ...
                for items in match:
                    url= match[0][1]
                    name=match[0][0]
                    name = name.encode('ascii', 'ignore').replace("&#39;","'") .replace("&amp;","and") .replace("&#252;","u") .replace("&quot;","").replace("[","").replace("]","").replace("-"," ")
                    iconimage = 'http://i.ytimg.com/vi/%s/0.jpg' % url
                    addLink(name,url,iconimage,'')        
        if '<span class="yt-uix-button-content">Next' in html:        
            match1=re.compile('href="(.+?)".+?">Next ').findall(html)
