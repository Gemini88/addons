...
def do_song_search(search_entered,iconimage):
    ...
    url='http://mp3skull.com.cyberunlocker.com/mp3/'+str(search_entered).replace('+','_')+'.html'
    link = OPEN_URL(url).replace('\n','')
    match=re.compile('-->.+?([0-9]*?) kbps<br />.+?div id="right_song">.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"').findall(link)
    print 'MP3 Skull========================================='+str(match)
    for bitrate,name,url in match:
        name='[COLOR green]MP3 SKULL[/COLOR]-%sKbps-%s'%(bitrate,name) 
        addLink(name,url,icon2,search_entered.replace('+',' '),'','','true')   
    url='http://musicspace.in/download.php?q='+str(search_entered)
    link = OPEN_URL(url).replace('\n','')
    match=re.compile('-->.+?([0-9]*?) kbps<br />.+?div id="right_song">.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"').findall(link)
    print 'MP3 Skull========================================='+str(match)
    for bitrate,name,url in match:
        name='[COLOR red]MUSIC SPACE[/COLOR]-%sKbps-%s'%(bitrate,name) 
        addLink(name,url,icon2,name_Download,'','','true')    
    url='http://mp3chief.com.cyberunlocker.com/search?q='+str(search_entered)+'&duration=1&size=1&bitrate=4'
    link = OPEN_URL(url)
    match=re.compile('<br>						(.+?) kbps<br>.+?div class="item_name">(.+?)</div>.+?<li class="download"><a href="(.+?)"',re.DOTALL).findall(link)
...
def for_download_or_playlist(name,artist):
    try:
        ...
        if ADDON.getSetting('bitrate')=='true':
                r_mp3skull='-->.+?320 kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;">.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"'
        else:
               r_mp3skull='kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;">.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"'
               
               
               
        url='http://mp3skull.com.cyberunlocker.com/mp3/'+str(artist).replace('+','_')+'_'+str(name).replace('+','_')+'.html'
        link = OPEN_URL(url).replace('\n','')
        match=re.compile(r_mp3skull).findall(link)
        print match
        try:
            url=match[0]
        ...
        if url=='':
            url='http://mp3chief.com.cyberunlocker.com/search?q='+str(name)+'&duration=1&size=1&bitrate=4'
            link = OPEN_URL(url)
            match=re.compile('div class="item_name">.+?</div>.+?<li class="download"><a href="(.+?)"',re.DOTALL).findall(link)
            try:
                url=match[0]
        ...
        if url=='':
            url='http://musicspace.in/download.php?q='+str(name)
            link = OPEN_URL(url)
...
def album_links(name,artist):
    ...
    link = OPEN_URL(url).replace('\n','')
    match=re.compile('-->.+?([0-9]*?) kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"').findall(link)
    print 'MP3 Skull========================================='+str(match)
    for bitrate,name,url in match:
        name='[COLOR green]MP3 SKULL[/COLOR]-%sKbps-%s'%(bitrate,name) 
        return_name.append(name)  
        return_url.append(url)   
    url='http://mp3chief.com.cyberunlocker.com/search?q='+str(name_url)+'&duration=1&size=1&bitrate=4'
    link = OPEN_URL(url)
    match=re.compile('<br>						(.+?) kbps<br>.+?div class="item_name">(.+?)</div>.+?<li class="download"><a href="(.+?)"',re.DOTALL).findall(link)
    print 'MP3 CHIEF========================================='+str(match)
    for bitrate,name,url in match:
        ...
        return_name.append(name)  
        return_url.append(url) 
    url='http://mp3skull.com.cyberunlocker.com/mp3/'+str(name_url).replace('+','_')+'.html'
    url='http://musicspace.in/download.php?q='+str(name_url)
    link = OPEN_URL(url).replace('\n','')
    match=re.compile('-->.+?([0-9]*?) kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"').findall(link)
    print 'MUSIC SPACE========================================='+str(match)
    for bitrate,name,url in match:
        name='[COLOR red]MUSIC SPACE[/COLOR]-%sKbps-%s'%(bitrate,name) 
        return_name.append(name)  
        return_url.append(url) 
...
def song_links(name,url,iconimage,fanart,artist,album):
    if ADDON.getSetting('oneclick')=='true':
        if url=='Number Ones':
            ...
            _artist = artist
    r_mp3skull='-->.+?([0-9]*?) kbps<br />.+?div id="right_song">.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"'
            
    name_Download=str(name)
    name_url=str(name).replace(' ','+').replace("'",'+').replace(',','').replace('(','').replace(')','').replace('?','')
    artist_url=str(artist).replace(' ','+').replace("'",'+').replace(',','').replace('(','').replace(')','').replace('?','')
    url='http://mp3skull.com.cyberunlocker.com/mp3/'+str(artist_url).replace('+','_')+'_'+str(name_url).replace('+','_')+'.html'
    print url
    link = OPEN_URL(url).replace('\n','')
    match=re.compile(r_mp3skull).findall(link)
    print 'MP3 Skull========================================='+str(match)
    ...
    print url
    link = OPEN_URL(url)
    match=re.compile(r_mp3skull,re.DOTALL).findall(link)
    print 'MUSIC SPACE========================================='+str(match)
    for bitrate,name,url in match:
        name='[COLOR red]MUSIC SPACE[/COLOR]-%sKbps-%s'%(bitrate,name) 
        addLink(name,url,icon2,name_Download,'','','true') 
